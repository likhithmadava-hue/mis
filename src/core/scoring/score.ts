...
// Module-scoped Intl.DateTimeFormat instances to avoid expensive allocations in hot loops
...
export const scoreRange = (days: number): ScoredDay[] => {
  ...
  // Map daily metrics by date for O(1) lookups instead of O(M) linear searches
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    return {
      date,
      label: weekdayFormatter.format(dObj).slice(0, 2),
      dateLabel: monthDayFormatter.format(dObj),
      metric,
      scores,
      byMode: scores
        ? {
