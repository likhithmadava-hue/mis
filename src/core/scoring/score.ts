...
export const scoreDay = (
  ...
  // Performance optimization: support Set<string> for O(1) membership checks during range evaluation
...
// Performance optimization: reuse module-scoped Intl.DateTimeFormat instances and empty set
// to avoid expensive object/formatter re-instantiations inside hot range-scoring loops.
const weekdayFormatter = new Intl.DateTimeFormat(undefined, { weekday: 'short' });
const dateFormatter = new Intl.DateTimeFormat(undefined, { day: 'numeric', month: 'short' });
const EMPTY_DONE_SET = new Set<string>();
...
export const scoreRange = (days: number): ScoredDay[] => {
  ...
  // Performance optimization: index daily metrics into a Map for O(1) lookups during range loops
  const metricsByDate = new Map<string, DailyMetric>();
  for (const m of metrics) {
    metricsByDate.set(m.date, m);
  }

  // Performance optimization: bucket habit log into Set<string> by date for O(1) habit lookup
  ...
  return Array.from({ length: days }, (_, i) => {
    ...
    const scores = metric ? scoreDay(metric, habits, doneByDate.get(date) ?? EMPTY_DONE_SET, user) : null;
