...
export const scoreRange = (days: number): ScoredDay[] => {
  ...
  // Performance optimization: Map daily metrics by date for O(1) lookup during score range iteration
  const metricsByDate = new Map<string, DailyMetric>(metrics.map((m) => [m.date, m]));
