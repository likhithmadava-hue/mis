...
// expensive repeated object instantiations when formatting dates in hot loops or render hooks.
...
export const scoreRange = (days: number): ScoredDay[] => {
  ...
  // for O(1) lookups during range loops, reducing time complexity from O(days * N) to O(days + N).
