 ...
 */
export default function DailyLog({ mode, triggerUpdate, onLogChange }: DailyLogProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card rounded-2xl border border-border card-shadow p-6 flex items-center gap-6">
        ...
        >
          <span className="text-3xl font-bold font-mono leading-none">{dayScore}</span>
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-lg font-bold font-space flex items-center gap-2">
            <meta.icon size={18} className="text-primary" /> Today’s {meta.label} Log
          </h3>
          <p className="text-xs text-muted-foreground mt-1.5 leading-relaxed">
            This is the only place you enter anything. Each track is scored out of 10, then weighted
            by the priority you give it — <span className="text-primary font-semibold">High</span> counts
            3×, Medium 2×, Low 1×. Raising a priority moves that card up and makes it matter more.
            {' '}<span className="text-primary font-semibold">{meta.label}</span> and the other mode
            score separately, so neither can drag the other down.
          </p>
        </div>
      </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-2.5 border-b border-border pb-3">
                ...
                <PriorityPicker value={priorities[id]} onChange={(p) => log.setTrackPriority(id, p)} />
                <span
