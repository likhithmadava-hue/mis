...
import { Check, Moon, Plus, Trash2 } from "lucide-preact";
...
export default function TrackControl({
  ...
  switch (id) {
      ...
      return (
        <div className="space-y-1">
          ...
          <input
            ...
            onChange={(e) =>
              patchToday({ study_hours: Number(e.currentTarget.value) })
            }
        ...
        <div className="flex gap-2">
          <label className="flex-1">
            ...
            <input
              ...
              onChange={(e) =>
                patchToday({ dpps_got: Number(e.currentTarget.value) })
              }
          ...
          <label className="flex-1">
            ...
            <input
              ...
              onChange={(e) =>
                patchToday({ dpps_complete: Number(e.currentTarget.value) })
              }
              className="w-full h-9 px-3 bg-background border border-border rounded-lg text-xs font-mono"
            />
          </label>
      ...
      return (
        <div className="space-y-1">
          ...
          <input
            ...
            onChange={(e) =>
              patchToday({ well_spent_time: Number(e.currentTarget.value) })
            }
      ...
      return (
        <div className="space-y-1">
          ...
          <input
            ...
            onChange={(e) =>
              patchToday({ mood_score: Number(e.currentTarget.value) })
            }
            className="w-full h-1.5 bg-muted rounded-lg appearance-none cursor-pointer accent-primary"
          />
        </div>
        ...
        <div className="space-y-3">
          <div className="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <p
                className={`text-[10px] font-mono ${today.water_count >= user.water_target ? "text-success" : "text-muted-foreground"}`}
              >
          ...
          <div className="flex gap-2 items-end border-t border-border pt-3">
            <Moon
              size={14}
              className="text-muted-foreground mb-2.5 flex-shrink-0"
            />
            <label className="flex-1">
              <span className="text-[9px] text-muted-foreground block mb-1">
                Bedtime
              ...
              <input
                ...
                onChange={(e) =>
                  log.saveSleep({ sleep_bedtime: e.currentTarget.value })
                }
            ...
            <label className="flex-1">
              ...
              <input
                ...
                onChange={(e) =>
                  log.saveSleep({ sleep_wake: e.currentTarget.value })
                }
                className="w-full h-9 px-3 bg-background border border-border rounded-lg text-xs [color-scheme:dark]"
              />
            </label>
          ...
          {habits.map((h) => {
            ...
            return (
              ...
              >
                ...
                <span
                  className={`flex-1 text-xs truncate ${done ? "text-muted-foreground line-through" : ""}`}
                >
                  {h.name}
                </span>
                <PriorityPicker
                  value={h.priority}
                  onChange={(p) => log.setHabitPriority(h.id, p)}
                />
                <button
    ...
    // honest for preact, which will not accept `undefined`
    default:
      return null;
  }
}
