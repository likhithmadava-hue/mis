...
function usePaperAnalytics(logbook: MarkLogbookEntry[]) {
  return useMemo(() => {
    ...
    const reasonCounts = logbook.reduce(
      (acc, e) => {
    ...
    const bySubject = new Map<
      string,
      { got: number; max: number; n: number }
    >();
    ...
    return {
      ...
      avgPaper: totalEntries
        ? Math.round(
            logbook.reduce((sum, e) => sum + (e.score / e.max_score) * 100, 0) /
              totalEntries,
          )
        : 0,
      reasonBars,
...
export function useGrowthData(
  ...
  const [logbook, setLogbook] = useState<MarkLogbookEntry[]>(
  ...
  const bestDay = logged.reduce<ScoredDay | null>(
    (best, d) =>
      !best || (d.byMode?.[mode] ?? 0) > (best.byMode?.[mode] ?? 0) ? d : best,
    null,
  );
  ...
  const studyProgress = Math.min(
    ((t?.study_hours ?? 0) / user.target_study_hours) * 100,
    100,
  );
  const dppTarget = t?.dpps_got ?? 0;
  const dppProgress =
    dppTarget > 0
      ? Math.min(((t?.dpps_complete ?? 0) / dppTarget) * 100, 100)
      : 0;
  ...
  return {
    ...
    avgScore: logged.length
      ? Math.round(
          logged.reduce((s, d) => s + (d.byMode?.[mode] ?? 0), 0) /
            logged.length,
        )
      : 0,
    totalStudy:
      Math.round(
        days.reduce((s, d) => s + (d.metric?.study_hours ?? 0), 0) * 10,
      ) / 10,
    ...
    academic: {
      studyBars: days.map((d) => ({
        ...
        className:
          (d.metric?.study_hours ?? 0) >= user.target_study_hours
            ? "bg-success"
            : "bg-primary",
      })),
      dppBars: days.map((d) => ({
    ...
    life: {
      ...
      leisureBars: days.map((d) => ({
        ...
        className:
          (d.metric?.well_spent_time ?? 0) > WELL_SPENT_TARGET * 2
            ? "bg-red-500"
            : "bg-yellow-500",
      })),
      waterBars: days.map((d) => ({
        ...
        className:
          (d.metric?.water_count ?? 0) >= user.water_target
            ? "bg-success"
            : "bg-primary",
      })),
      postureBars: days.map((d) => ({
        ...
        className:
          (d.metric?.posture_count ?? 0) >= POSTURE_TARGET
            ? "bg-success"
            : "bg-emerald-600",
      })),
    },
