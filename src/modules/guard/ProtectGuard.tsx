          ...
          <div className="grid grid-cols-2 gap-2">
            {user.blocked_apps.map(app => (
              <div key={app} className="p-3 bg-background border border-border rounded-xl flex items-center justify-between">
                ...
                <button onClick={() => handleDeleteApp(app)} aria-label={`Remove ${app} from blocklist`} title={`Remove ${app}`} className="text-muted-foreground hover:text-destructive p-1 rounded-md"><Trash2 size={13} /></button>
              </div>
            ))}
          </div>
        </div>
