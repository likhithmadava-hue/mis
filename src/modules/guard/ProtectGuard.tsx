        ...
        <div className="space-y-4">
          ...
          <form onSubmit={handleAddApp} className="flex gap-2">
            <input
              ...
              aria-label="New domain to shield"
              value={newAppText}
              onChange={(e) => setNewAppText(e.target.value)}
              className="flex-1 px-3 py-2 bg-background border border-border rounded-xl text-xs focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
            />
            <button
              type="submit"
              className="px-4 py-2 bg-primary text-primary-foreground font-semibold rounded-xl text-xs flex items-center gap-1 focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none transition-colors"
            >
            ...
            {user.blocked_apps.map(app => (
              <div key={app} className="p-3 bg-background border border-border rounded-xl flex items-center justify-between">
                ...
                <button
                  ...
                  className="p-1 rounded-md text-muted-foreground hover:text-destructive focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none transition-colors"
                >
                  <Trash2 size={13} />
                </button>
              </div>
        ...
        <div className="space-y-4">
          ...
          <form onSubmit={handleTestBlocker} className="space-y-3">
            <input
              ...
              aria-label="Test application domain"
              required
              value={blockedTestDomain}
              onChange={(e) => setBlockedTestDomain(e.target.value)}
              className="w-full px-3 py-2 bg-card border border-border rounded-xl text-xs focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none"
            />
            <button
              type="submit"
              className="w-full py-2 bg-muted hover:bg-muted/80 text-foreground text-xs font-semibold rounded-xl border border-border focus-visible:ring-2 focus-visible:ring-primary focus-visible:outline-none transition-colors"
            >
              Inject Request
            </button>
          </form>
