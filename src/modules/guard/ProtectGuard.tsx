      ...
      <div className="lg:col-span-7 bg-card rounded-2xl border border-border p-6 flex flex-col justify-between space-y-6">
        <div className="space-y-4">
          ...
          <form onSubmit={handleAddApp} className="flex gap-2">
            <input
              type="text"
              aria-label="New domain or app to shield"
              placeholder="e.g. TikTok, Netflix"
              value={newAppText}
              onChange={(e) => setNewAppText(e.target.value)}
              className="flex-1 px-3 py-2 bg-background border border-border rounded-xl text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            />
            <button
              type="submit"
              className="px-4 py-2 bg-primary text-primary-foreground font-semibold rounded-xl text-xs flex items-center gap-1 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
            ...
            {user.blocked_apps.map(app => (
              <div key={app} className="p-3 bg-background border border-border rounded-xl flex items-center justify-between">
                ...
                <button
                  ...
                  aria-label={`Unshield ${app}`}
                  title={`Unshield ${app}`}
                  className="p-1.5 -mr-1 rounded-lg text-muted-foreground hover:text-destructive hover:bg-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring transition-colors"
                >
                  <Trash2 size={13} />
                </button>
              </div>
      ...
      <div className="lg:col-span-5 bg-card rounded-2xl border border-border p-6 flex flex-col justify-between">
        <div className="space-y-4">
          ...
          <form onSubmit={handleTestBlocker} className="space-y-3">
            <input
              type="text"
              aria-label="Domain or app to test in blocker simulator"
              placeholder="Type application/domain..."
              required
              value={blockedTestDomain}
              onChange={(e) => setBlockedTestDomain(e.target.value)}
              className="w-full px-3 py-2 bg-card border border-border rounded-xl text-xs focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            />
            <button
              type="submit"
              className="w-full py-2 bg-muted hover:bg-muted/80 text-foreground text-xs font-semibold rounded-xl border border-border focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              Inject Request
            </button>
          </form>
