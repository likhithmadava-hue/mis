...
export default function ProtectGuard({
  triggerUpdate,
  onBlocklistChange,
}: ProtectGuardProps) {
  ...
  const handleAddApp = (e: SubmitEvent) => {
    e.preventDefault();
  ...
  const handleTestBlocker = (e: SubmitEvent) => {
    e.preventDefault();
    if (!blockedTestDomain.trim()) return;

    const testApp = blockedTestDomain.trim().toLowerCase();
    const match = user.blocked_apps.some((app) =>
      testApp.includes(app.toLowerCase()),
    );
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between border-b border-border pb-3">
            <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground font-space flex items-center gap-2">
              <Smartphone size={16} /> Perimeter Accessibility Controls
            </h3>
            <span
              className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded-full flex items-center gap-1 ${user.is_focus_active ? "bg-red-500/20 text-red-400" : "bg-emerald-500/20 text-emerald-400"}`}
            >
              {user.is_focus_active ? "Focus Locked" : "Guard Active"}
            </span>
          </div>
          <form onSubmit={handleAddApp} className="flex gap-2">
            <input
              ...
              value={newAppText}
              onChange={(e) => setNewAppText(e.currentTarget.value)}
              className="flex-1 px-3 py-2 bg-background border border-border rounded-xl text-xs"
            />
            <button
              type="submit"
              className="px-4 py-2 bg-primary text-primary-foreground font-semibold rounded-xl text-xs flex items-center gap-1"
            >
          ...
          <div className="grid grid-cols-2 gap-2">
            {user.blocked_apps.map((app) => (
              <div
                key={app}
                className="p-3 bg-background border border-border rounded-xl flex items-center justify-between"
              >
                <span className="text-xs font-semibold flex items-center gap-2">
                  <EyeOff size={13} className="text-primary" /> {app}
                </span>
                <button
        ...
        <div className="p-4 bg-primary/5 border border-primary/10 rounded-xl">
          <div className="text-[11px] text-muted-foreground font-mono space-y-1">
            <p className="text-emerald-400">
              ● [Accessibility Hook] Engaged successfully.
            </p>
            <p>
              ● [Lockout Policy]{" "}
              {user.is_focus_active
                ? "FORCE STRICTOR ENGAGED"
                : "Awaiting focus sprint."}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="lg:col-span-5 bg-card rounded-2xl border border-border p-6 flex flex-col justify-between">
        <div className="space-y-4">
          <h3 className="text-sm font-bold uppercase tracking-wider text-muted-foreground font-space flex items-center gap-2">
            <ShieldAlert size={16} /> Perimeter Blocker Simulator
          </h3>
          <form onSubmit={handleTestBlocker} className="space-y-3">
            <input
              ...
              onChange={(e) => setBlockedTestDomain(e.currentTarget.value)}
              className="w-full px-3 py-2 bg-card border border-border rounded-xl text-xs"
            />
            <button
              type="submit"
              className="w-full py-2 bg-muted hover:bg-muted/80 text-foreground text-xs font-semibold rounded-xl border border-border"
            >
              Inject Request
            </button>
          </form>
          {testResult.status && (
            <div
              className={`p-4 rounded-xl border ${testResult.status === "blocked" ? "bg-red-500/10 border-red-500/20 text-red-400 border-glow" : "bg-emerald-500/10 border-emerald-500/20 text-emerald-400"}`}
            >
