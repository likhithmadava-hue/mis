...
import { confirmDialog, EmptyState, messageDialog, Workspace } from '../../core/ui';
...
export default function DatabaseExplorer() {
  ...
  const handleDelete = async (entry: MarkLogbookEntry) => {
    const yes = await confirmDialog({
      title: 'Delete record',
      body: `Delete the ${entry.subject} — ${entry.chapter || 'entry'} record? This cannot be undone.`,
      tone: 'danger',
      confirmLabel: 'Delete',
    });
  ...
  const handleImport = async (file: File) => {
    ...
    try {
      ...
      const yes = await confirmDialog({
        ...
        body: `Replace all ${entries().length} records with ${parsed.length} from the backup?`,
        tone: 'danger',
      ...
      await messageDialog({
        ...
        body: `A .json import has to be a backup MIS exported.\n\n${errorMessage(e)}`,
        tone: 'danger',
      });
    }
  };
  ...
  return (
    <div class="flex-1 flex flex-col bg-card rounded-2xl border border-border card-shadow p-4 sm:p-6">
      <Workspace
        header={
          <div class="space-y-5">
            <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-border pb-4">
              <div>
                ...
                <p class="text-xs text-muted-foreground mt-1 max-w-xl">
                  Search, filter, edit. The raw record of everything you've gotten wrong — and
                  learned from. Import an Excel sheet or CSV you already keep, and it folds straight
                  in.
                </p>
              </div>
      ...
      >
        <div class="w-full h-full max-h-[70dvh] overflow-auto bg-background rounded-xl border border-border">
          <Show
            ...
            fallback={
              <EmptyState
                ...
                message={
                  entries().length === 0
                    ? 'Add one from the Daily Log, or import a spreadsheet you already keep.'
                    : 'Loosen a filter, or clear them all to see every record again.'
                }
                action={
                  entries().length === 0
                    ? { label: 'Open the Daily Log', to: 'log' }
