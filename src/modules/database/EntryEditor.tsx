...
export default function EntryEditor({
  ...
  const set = (patch: Partial<MarkLogbookEntry>) =>
      ...
      <td colSpan={9} className="p-4">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 animate-fade-in">
          ...
          <label className="block">
            ...
            <input
              type="number"
              value={draft.time_spent}
              onChange={(e) => set({ time_spent: Number(e.target.value) })}
              className={`w-full ${fieldClass}`}
            />
          </label>
          <label className="block">
        ...
        <div className="flex justify-end gap-2 mt-3">
          ...
          <button
            onClick={onSave}
            className="px-4 py-1.5 bg-primary text-primary-foreground rounded-lg text-xs font-semibold font-space flex items-center gap-1"
          >
            <Check size={13} /> Save
          </button>
        </div>
