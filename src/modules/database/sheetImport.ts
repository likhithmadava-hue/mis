...
/**
 * Prevents CSV / Excel formula injection by prefixing string values starting
 * with `=`, `+`, `-`, `@`, `\t`, or `\r` with a single quote (`'`).
 */
export function sanitizeFormula(val: unknown): string {
  const s = String(val ?? '');
  if (/^[=+\-@\t\r]/.test(s)) {
    return `'${s}`;
  }
