...
export default function LeftTodayPanel({ remaining, allClear }: LeftTodayPanelProps) {
  return (
    <div className="bg-card rounded-2xl border border-border card-shadow p-6 space-y-4">
      ...
      <div className="space-y-3">
        {remaining.map(({ key, label, icon: Icon, left, total, unit, done }) => {
          ...
          return (
            <div key={key} className="space-y-1.5">
              <div className="flex items-center gap-2">
                <Icon size={14} className={cleared && !nothingSet ? 'text-success' : 'text-muted-foreground'} />
                <span className="text-xs flex-1 min-w-0 truncate">{label}</span>
                <span
                  className={`text-sm font-bold font-mono flex-shrink-0 ${
                    nothingSet ? 'text-muted-foreground/50' : cleared ? 'text-success' : 'text-foreground'
                  }`}
                >
