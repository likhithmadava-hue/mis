...
export default function FocusMusicPanel(props: FocusMusicPanelProps) {
  const off = () => props.music.kind() === 'off';
  ...
  return (
    <div class="bg-card rounded-2xl border border-border card-shadow p-6 space-y-4">
      ...
      {/* Always drawn, dimmed while Off. If this row only appeared once a style
          was picked, the card would grow by its height and shove every card
          below it down the column — Left Today included. */}
      <div
        class={`flex items-center gap-3 transition-opacity ${off() ? 'opacity-50' : ''}`}
        aria-disabled={off()}
      >
        <button
          ...
          class="p-2.5 rounded-xl bg-primary text-primary-foreground flex-shrink-0 disabled:cursor-not-allowed"
        >
        ...
        <span
          ...
          title={props.music.trackTitle() ?? undefined}
        >
          {off() ? 'Pick a style to start' : (props.music.trackTitle() ?? 'Not playing')}
        </span>

        <div class="flex items-center gap-1.5 flex-shrink-0">
          <Show
            when={props.music.volume() > 0}
